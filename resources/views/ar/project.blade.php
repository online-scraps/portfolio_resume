                                ...
                                <form action="{{ route('admin.projects.update', $project->id) }}" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="p-5 grid grid-cols-12 gap-4 row-gap-3">
                                        <div class="col-span-12 sm:col-span-12"> <label>Name</label> <input type="text"
                                                class="input w-full border mt-2 flex-1" placeholder="Project name" name="name" value="{{ $project->name }}">
                                        </div>
                                        <div class="col-span-12 sm:col-span-12"> <label>Description</label> <input type="text"
                                                class="input w-full border mt-2 flex-1" placeholder="Important Meeting" name="description" value="{{  $project->description }}"> </div>
                                        <div class="col-span-12 sm:col-span-6"> <label>Category</label> <select
                                                ...
                                                @foreach (App\Models\Projects::projectCategoryType() as $key => $value)
                                                    <option value="{{ $key }}" {{ $project->category_id == $key ? 'selected' : '' }}>{{ $value }}</option>
                                                @endforeach
                                            </select> </div>
                                            <div class="col-span-12 sm:col-span-6"> <label>Link</label> <input type="text"
                                                class="input w-full border mt-2 flex-1" placeholder="Link for this project" name="link" value="{{ $project->link }}">
                                        </div>
                                    </div>
                                    <div class="px-5 py-3 text-right border-t border-gray-200 dark:border-dark-5 text-center" text-center> <button
                                            type="button"
                                            class="button w-20 border text-gray-700 dark:border-dark-5 dark:text-gray-300 mr-1"
                                            data-dismiss="modal">Cancel</button>
                                        <button type="submit" class="button w-100 bg-theme-12 text-white"> Update Project</button>
                                    </div>
                                </form>
                            </div>
                        </div>
    ...
    <div class="modal" id="create-modal">
        <div class="modal__content modal__content--lg ">
            ...
            <form action="{{ route('admin.projects.store') }}" method="post" enctype="multipart/form-data">
                ...
                <div class="p-5 grid grid-cols-12 gap-4 row-gap-3">
                    <div class="col-span-12 sm:col-span-12"> <label>Name</label> <input type="text"
                            class="input w-full border mt-2 flex-1" placeholder="Project name" name="name" value="{{ old('name') }}">
                    </div>
                    <div class="col-span-12 sm:col-span-12"> <label>Description</label> <input type="text"
                            class="input w-full border mt-2 flex-1" placeholder="Important Meeting" name="description"> </div>
                    <div class="col-span-12 sm:col-span-6"> <label>Category</label> <select
                        ...
                        <div class="col-span-12 sm:col-span-6"> <label>Link</label> <input type="text"
                            class="input w-full border mt-2 flex-1" placeholder="Link for this project" name="link" value="{{ old('link') }}">
                    </div>
                </div>
                <div class="px-5 py-3 text-right border-t border-gray-200 dark:border-dark-5 text-center" text-center> <button
                        type="button"
